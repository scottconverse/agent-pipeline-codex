...
- Never end a turn, defer, skip push, skip CI, write a stopping handoff, compact-and-stop, or ask a non-gate question during an authorized pipeline run unless `.agent-runs/<run-id>/active-control-state.md` records a valid stop condition, the shared `scripts/policy/stop_validator.py` can prove it from current run evidence, `python scripts/policy/check_pipeline_control_loop.py --run <run-id>` passes, `python scripts/policy/final_response_gate.py --require-active-run` prints `final_response_gate: ALLOW`, and `python scripts/policy/agent_decision_gate.py --intent <intent> --claimed-stop-condition <condition> --write-ledger` allows that specific decision.
...
- Treat execute-stage DoD readiness as a hard pre-verify gate. The runner must not log `execute | COMPLETE` or advance to policy/verify unless `implementation-report.md` contains `**DoD readiness: READY**` and a parseable `**DoD checklist: T total, R ready, B blocked, D deferred**` line with zero blocked items. If `scripts/policy/check_execute_readiness.py` exists, it must exit 0 too; if the script is absent in an older project, enforce the same parse manually and update the project plumbing before promotion.
