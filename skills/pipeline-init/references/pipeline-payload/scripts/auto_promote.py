...
Auto-promote is eligible only when ALL of the following are true:
  ...
  1. `verifier-report.md` exists and its Â§0 count line shows zero
     NOT MET and zero PARTIAL criteria.
  2. `critic-report.md` exists and its Â§2 count line shows zero
     blocker and zero critical findings.
  3. `drift-report.md` exists and its Â§2 count line shows zero blocker
     drift items.
...
def _check_verifier(run_dir: Path) -> ConditionResult:
    ...
    if not m:
        return ConditionResult(
            "verifier-clean",
            False,
            "verifier-report.md missing or malformed Â§0 criteria count line "
    ...
    return ConditionResult(
        "verifier-clean",
        True,
        f"verifier-report.md Â§0: {total} total criteria, {met} MET, {na} NOT APPLICABLE, 0 PARTIAL, 0 NOT MET.",
    )
...
def _check_critic(run_dir: Path) -> ConditionResult:
    ...
    if not m:
        return ConditionResult(
            "critic-clean",
            False,
            "critic-report.md missing or malformed Â§2 findings count line "
    ...
    return ConditionResult(
        "critic-clean",
        True,
        f"critic-report.md Â§2: {total} findings ({blocker} blocker, {critical} critical, {major} major, {minor} minor).",
    )
...
def _check_drift(run_dir: Path) -> ConditionResult:
    ...
    if not m:
        return ConditionResult(
            "drift-clean",
            False,
            "drift-report.md missing or malformed Â§2 drift count line "
    ...
    return ConditionResult(
        "drift-clean",
        True,
        f"drift-report.md Â§2: {total} drift item(s), 0 blocker.",
    )
...
def _write_decision(run_dir: Path, conditions: list[ConditionResult]) -> None:
    ...
    lines = [
        "**Decision: PROMOTE**",
        "",
        "_Generated by `scripts/auto_promote.py` â€” all six v0.5 auto-promote conditions satisfied. "
    ...
    for c in conditions:
        ...
        lines.append(f"- **{marker}** `{c.name}` â€” {c.evidence}")
    lines.extend(
        [
            "",
            "## Disposition",
            "",
            "PROMOTE â€” proceed to merge per the manifest's `required_gates`. The final "
...
def main() -> int:
    ...
    if not run_dir.is_dir():
        print(f"auto_promote: FAIL â€” run directory not found at {run_dir}", file=sys.stderr)
        return 2
    ...
    for c in conditions:
        marker = "PASS" if c.passed else "FAIL"
        print(f"  [{marker}] {c.name} â€” {c.evidence}")

    if all_passed:
        _write_decision(run_dir, conditions)
        print("auto_promote: ELIGIBLE â€” manager-decision.md written with PROMOTE.")
        return 0

    _write_report(run_dir, conditions)
    print("auto_promote: NOT_ELIGIBLE â€” see auto-promote-report.md; manager stage will run with human gate.")
    return 1
